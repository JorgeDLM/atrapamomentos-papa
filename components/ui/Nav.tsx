...
export default function Nav() {
  ...
  const base = `/${locale}`
  // Only the landing page has a dark hero behind the transparent nav. On every
  // interior page (light background) the nav must be solid from the start so the
  // links stay legible without scrolling.
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-[400ms] ${
        solid ? 'bg-ivory/95 backdrop-blur-sm border-b border-stone-warm/20' : ''
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-6 flex justify-between items-center">
        <Link
          ...
          className={`font-serif text-base tracking-wide hover:opacity-70 transition-all duration-[400ms] ${
            solid ? 'text-stone-dark' : 'text-ivory'
          }`}
        >
          Jorge de la Mora
        </Link>
